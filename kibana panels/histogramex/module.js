/** @scratch /panels/5
 *
 * include::panels/histogramex.asciidoc[]
 */

/** @scratch /panels/histogramex/0
 *
 * == HistogramEx
 * Status: *Stable*
 *
 * The histogramex panel allow for the display of time charts. It includes several modes and tranformations
 * to display event counts, mean, weighted-mean, min, max and total of numeric fields, and derivatives of counter
 * fields.
 *
 */

define("panels/histogramex/module", [
  'angular',
  'app',
  'jquery',
  'lodash',
...
function (angular, app, $, _, kbn, moment, timeSeries, numeral) {
  ...
  var module = angular.module('kibana.panels.histogramex', []);
  app.useModule(module);

  module.controller('histogramex', function($scope, querySrv, dashboard, filterSrv) {
	//  alert("in controller");
    $scope.panelMeta = {
      modals : [
        {
      ...
      editorTabs : [
        {
          title:'Style',
          src:'app/panels/histogramex/styleEditor.html'
        },
        {
          title:'Queries',
          src:'app/panels/histogramex/queriesEditor.html'
        },
      ],
      status  : "Stable",
    ...
    var _d = {
      ...
      /** @scratch /panels/histogram/3
       * value_field_count:: weight-mean computed field if +mode+ is set to weighted-mean or all. Must be numeric.
       */
      value_field_count   : null,
      
      /** @scratch /panels/histogram/3
       * value_field_min:: y-axis field for minimum if +mode+ is set to all. Must be numeric.
       */
      value_field_min   : null,
	  
      /** @scratch /panels/histogram/3
       * value_field_max:: y-axis field for maximum if +mode+ is set to all. Must be numeric.
       */
      value_field_max   : null,
	  
      /** @scratch /panels/histogram/3
       * value_field_threshold:: sets the threshold value to show on the y-axis. Must be numeric.
       */
      value_field_threshold : null,
	 
      /** @scratch /panels/histogram/3
       * value_field_threshold_color:: sets the threshold color. Must be hex or string color value.
       */
      value_field_threshold_color : 'red',
    ...
    $scope.panel.showchart = {"max":true,"weighted-mean":true,"min":true};
    ...
    $scope.get_data = function(data, segment, query_id) {
      ...
      // if panel.mode = all
      if( $scope.panel.mode === 'all' && queries.length > 0 ){
    	  //alert("doing all");
    	  if(_.isNull($scope.panel.value_field) ||
    		 _.isNull($scope.panel.value_field_min) || 
			 _.isNull($scope.panel.value_field_max) ) {
				$scope.panel.error = "In " + $scope.panel.mode + " mode fields must be specified for min, max, and mean";
				return;
			} // end if
			
    	  /*
    	  	// copy query q0 for mean
    	  	var q0 = $.extend(true, {}, queries[0]);
			q0.id=0;
			q0.color = "green";
			q0.alias = "weighted-mean";
			
			// copy query q0 for min
    	  	var q1 = $.extend(true, {}, queries[0]);
			q1.id=1;
			q1.color = "blue";
			q1.alias = "min";
			
			// copy query q0 for max
    	  	var q2 = $.extend(true, {}, queries[0]);
			q2.id=2;
			q2.color = "orange";
			q2.alias = "max";
			
			// store them and use them as queries
			queries = [q0,q1,q2]; 
			var mode_fields = [$scope.panel.value_field,$scope.panel.value_field_min, $scope.panel.value_field_max];
			var allmodes = ['weighted-mean','min','max'];
			*/
			...
			// store them and use them as queries
			queries = [q0,q1,q2]; 
			var mode_fields = [$scope.panel.value_field_max, $scope.panel.value_field,$scope.panel.value_field_min];
			var allmodes = ['max','weighted-mean','min'];
      ...
      _.each(queries, function(q) {
    	...
    	if( allmodes ){  // $scope.panel.mode === 'all')
    		//alert(q.id)
			$scope.panel.mode = allmodes[q.id];
			$scope.panel.value_field = mode_fields[q.id];
			//alert( $scope.panel.mode );	
		} 
		...
		// weighted average
		 	...
		 	if(_.isNull($scope.panel.value_field) || 
			   _.isNull($scope.panel.value_field_count) ) {
            	$scope.panel.error = "In " + $scope.panel.mode + " mode both fields must be specified";
            	return;
          	}
			var script = "doc['"+$scope.panel.value_field+"'].value * doc['"+$scope.panel.value_field_count+"'].value";
			facet = facet.keyField($scope.panel.time_field).valueScript(script).global(true);	
			// create a count facet 
			var cnt_facet = $scope.ejs.DateHistogramFacet("cnt_" + q.id);
			cnt_facet = cnt_facet.keyField($scope.panel.time_field).valueField($scope.panel.value_field_count).global(true);
			cnt_facet = cnt_facet.interval(_interval).facetFilter($scope.ejs.QueryFilter(query));
			request = request.facet(cnt_facet).size($scope.panel.annotate.enable ? $scope.panel.annotate.size : 0);
		} // end else
        
     // all
		//else if( $scope.panel.mode === 'all' ){			
		//	facet = facet.keyField($scope.panel.time_field).valueField(mode_fields[q.id]).global(true);
		//} // end else if
	  ...
	  if( allmodes ){  
	  		...
	  		$scope.panel.value_field = mode_fields[0];
	  }
      ...
      return results.then(function(results) {
        ...
        // Make sure we're still on the same query/queries
          ...
          _.each(queries, function(q) {
			...
			// weighted-mean - get the entries from the count facet
			if( $scope.panel.mode === 'weighted-mean' ){
				var cnt_entries = results.facets["cnt_"+q.id].entries;
			}
            ...
            _.each(query_results.entries, function(entry, index) {
              var value;
			  ...
			  // weighted-mean - do the weight-mean computation
			  else if ($scope.panel.mode === 'weighted-mean'){
				var cnt_entry = cnt_entries[index];
				var total = entry.total / cnt_entry.total;
			  	value = (time_series._data[entry.time] || 0) + total;
			  } 
			  
              time_series.addValue(entry.time, value);
            });

            // all mode switch back the mode to all
  		  	if( allmodes ) { 
  		  		$scope.panel.mode = "all";
  		  	}
  ...
  module.directive('histogramexChart', function(dashboard, filterSrv) {
	  return {
      restrict: 'A',
      template: '<div></div>',
        ...
        function render_panel(data) {
          ...
          try {
              ...
              grid: {
                ...
                hoverable: true,
                color: '#c8c8c8',
                //markings: [{ yaxis: { from: 10, to: 10 }, color: "#ff0000" }],
              aboveData:true
            ...
            // thershold values for the grid
            if(scope.panel.value_field_threshold != null ){
            	var t = scope.panel.value_field_threshold * scope.panel.scale;
            	var c = scope.panel.value_field_threshold_color;
            	options.grid.markings=[{ yaxis: { from: t, to: t }, color: c }];
          	} // end if
            ...
            if( scope.panel.mode == 'all' ){
            	var i=0;
            	for( var mode in scope.panel.showchart ){
            		//alert(mode);
            		if( scope.panel.showchart[mode] == false )	
            			data[i].data = null;
            		i++;
            	} // end for
            } // end if
